...
import {getCropAspect, parseImageAssetDimensions, TailImageValue} from './tailImageCrop'
import type {FixedAspectCropOptions, FixedAspectCropResult} from './tailImageCrop'
import {uploadRotatedImageAsset} from './imageRotation'

type FixedAspectImageInputProps = ObjectInputProps & {
  onClose?: () => void
  onPathBlur?: (path: unknown[]) => void
  path?: unknown[]
}

type FixedAspectImageInputConfig = {
  targetAspect: number
  frameSize: {
    width: number
    height: number
  ...
  isCropValid: (value?: TailImageValue) => boolean
}

const clamp = (value: number, min: number, max: number) => Math.min(Math.max(value, min), max)

const getZoomFromCrop = (value: TailImageValue | undefined, targetAspect: number) => {
  ...
  const baseWidth = imageAspect > targetAspect ? targetAspect / imageAspect : 1
  const baseHeight = imageAspect > targetAspect ? 1 : imageAspect / targetAspect
...
export function createFixedAspectImageInput(config: FixedAspectImageInputConfig) {
  function FixedAspectImageInput(props: ObjectInputProps) {
    const {onChange, value} = props
    const {onClose, onPathBlur, path} = props as FixedAspectImageInputProps
    ...
    const isValid = hasImage && config.isCropValid(imageValue)
    ...
    useEffect(() => {
      ...
      setZoom(getZoomFromCrop(imageValue, config.targetAspect))
      // Reset editor state only for a newly selected asset, not while its crop is adjusted.
      // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [assetRef])

    const applyCrop = (nextFocusX = focusX, nextFocusY = focusY, nextZoom = zoom) => {
      const nextCrop = config.createCrop(imageValue, {
        focusX: nextFocusX,
        focusY: nextFocusY,
        zoom: nextZoom,
      })
    ...
    const handleRotateClockwise = async () => {
      ...
      try {
        ...
        const nextCrop = config.createCrop(nextImage, {focusX: 0.5, focusY: 0.5, zoom: 1})
                ...
                <Box>
                  <div
                    style={{
                      ...
                      overflow: 'hidden',
                      width: '100%',
                      maxWidth: config.frameSize.width,
                    }}
                  >
                    {previewUrl && (
                      <img
